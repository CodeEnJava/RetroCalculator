package codeEnJava.maquette_cal_part_01;
...
public class Maquette_01 implements ConfigKeyboard {
	...
	public static void main(String[] args) {
		JFrame frame = new JFrame("Maquuette: calculatrice RETRO 01");
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.setLayout(new FlowLayout(0,10,10));
		
		// Définir les deux displays
		...
		int panel_height = (int)(ROWS*KEYHEIGHT+6*grid2d_height+display.getDimDisplay().getHeight()+
				                 displayResult.getDimDisplay().getHeight());
		
		frame.pack();// pour initialiser correctement les bordures
		...
		matrice.setBackgroundColor("#1A2A3F");
		matrice.setBorderColor("#1A2A3F");
